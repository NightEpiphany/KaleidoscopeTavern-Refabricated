...
public class LavaCauldronTapBehavior implements ITapBehavior {
    ...
    public boolean isMatch(
        ...
        if (ConfigGetter.getInfiniteLavaFromTap()) {
            // 如果配置允许无限水龙头取熔岩，那么只要下方是炼药锅或者空瓶就行
            return destinationState.is(Blocks.CAULDRON) || destinationState.is(ModBlocks.EMPTY_BOTTLE);
        }
        // 否则只能取燃烧瓶
        return destinationState.is(ModBlocks.EMPTY_BOTTLE);
    }
    ...
    public ParticleOptions onStartExtract(
        ...
        return ModParticles.LAVA_TAP_DRIP;
    }
    ...
    public void onEndExtract(
        ...
        // 如果下方是空炼药锅
        if (ConfigGetter.getInfiniteLavaFromTap() && destinationState.is(Blocks.CAULDRON)) {
        ...
        // 下方是空瓶
        if (destinationState.is(ModBlocks.EMPTY_BOTTLE)) {
            level.setBlockAndUpdate(tapPos.below(), ModBlocks.MOLOTOV.defaultBlockState());
