    ...
    public void appendTooltip(ITooltip iTooltip, BlockAccessor blockAccessor, IPluginConfig iPluginConfig) {
        ...
        if (fluidId.isEmpty() || amount <= 0) {
            iTooltip.add(helper.progress(ratio, Component.literal("Liquid: Empty " + amount + "/" + capacity + " mB").withStyle(ChatFormatting.WHITE),
                    helper.progressStyle(), BoxStyle.getNestedBox(), false));
            return;
        }
        ...
        FluidVariant variant = nbt == null ? FluidVariant.of(fluid) : FluidVariant.of(fluid, DataComponentPatch.builder().set(DataComponents.CUSTOM_DATA, CustomData.of(nbt)).build());
        if (!variant.isBlank()) {
            ...
            iTooltip.add(helper.progress(ratio,
                    ...
                    helper.progressStyle().overlay(nbt == null ? helper.fluid(JadeFluidObject.of(fluid, amount))
                            : helper.fluid(JadeFluidObject.of(fluid, amount, DataComponentPatch.builder().set(DataComponents.CUSTOM_DATA, CustomData.of(nbt)).build()))),
                    BoxStyle.getNestedBox(), false));
        } else {
            iTooltip.add(helper.progress(ratio, Component.literal("Liquid: Empty " + amount + "/" + capacity + " mB").withStyle(ChatFormatting.WHITE),
                    helper.progressStyle(), BoxStyle.getNestedBox(), false));
        }
