...
import com.github.ysbbbbbb.kaleidoscopetavern.mixin.ZombieVillagerAccessor;
...
public class IncenseBlockEntity extends BaseBlockEntity {
    public IncenseBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlocks.INCENSE_BE, pos, state);
    }
    ...
    public static void serverTick(Level level, BlockPos pos, BlockState state, IncenseBlockEntity incense) {
        ...
        for (LivingEntity entity : entities) {
            // 僵尸村民：生命 ≤ 1 时治愈，否则继续伤害
            if (entity instanceof ZombieVillager zombieVillager) {
                if (zombieVillager.isConverting()) {
                    continue;
                }
                if (zombieVillager.getHealth() <= 1.0f) {
                    ((ZombieVillagerAccessor) zombieVillager).invokeStartConverting(null, 60);
                    continue;
                }
            }
