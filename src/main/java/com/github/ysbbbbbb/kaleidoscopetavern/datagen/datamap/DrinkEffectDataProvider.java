...
public class DrinkEffectDataProvider implements DataProvider {
    ...
    private void addEntry() {
        ...
        add(ModItems.CARIGNAN,
                List.of(effect(MobEffects.HEALTH_BOOST, 0, 0)),
                List.of(effect(MobEffects.HEALTH_BOOST, 0, 1)),
                List.of(effect(MobEffects.HEALTH_BOOST, 0, 2)),
                List.of(effect(MobEffects.HEALTH_BOOST, 0, 3))
        );
        ...
        // 伏特加
        add(ModItems.VODKA,
                List.of(effect(MobEffects.INSTANT_DAMAGE, 80, 0)),
                List.of(effect(MobEffects.INSTANT_DAMAGE, 240, 0)),
                List.of(effect(MobEffects.INSTANT_DAMAGE, 240, 1)),
                List.of(effect(MobEffects.INSTANT_DAMAGE, 540, 1))
        );
     ...
     * 鸡尾酒没有酿造等级，只有单层效果
     */
    private void addCocktail(Item key, DrinkEffectData.Entry... entries) {
        var itemKey = BuiltInRegistries.ITEM.getKey(key);
    ...
    @SafeVarargs
    private void addWithLevel2Effects(Item key, List<DrinkEffectData.Entry>... levelAbove1) {
        var itemKey = BuiltInRegistries.ITEM.getKey(key);
        this.addWithLevel2Effects(itemKey.getPath(), key, levelAbove1);
    }

    @SafeVarargs
    private void addWithLevel2Effects(String fileName, Item key,
    ...
    public @NonNull CompletableFuture<?> run(@NonNull CachedOutput cache) {
        ...
        for (var entry : data.entrySet()) {
            DrinkEffectData.CODEC
                    ...
                    .ifPresent(json -> {
                        var filePath = pathProvider.json(Identifier.fromNamespaceAndPath(KaleidoscopeTavern.MOD_ID, entry.getKey()));
