...
public class BarrelBlockEntityRender implements BlockEntityRenderer<BarrelBlockEntity, BarrelBlockEntityRenderState> {
    ...
    public void extractRenderState(BarrelBlockEntity blockEntity, BarrelBlockEntityRenderState blockEntityRenderState, float f, @NonNull Vec3 vec3, ModelFeatureRenderer.@Nullable CrumblingOverlay crumblingOverlay) {
        ...
        ItemStackHandler items = blockEntity.getIngredient();
        blockEntityRenderState.itemStates = new ArrayList<>();
        for (int index = 0; index < items.getSlots(); index++) {
            ItemStack stack = items.getStackInSlot(index);
            if (!stack.isEmpty()) {
                BarrelBlockEntityRenderState.ItemEntry entry = new BarrelBlockEntityRenderState.ItemEntry();
                this.itemModelResolver.updateForTopItem(entry.itemState, stack, ItemDisplayContext.FIXED, null, null, (int) (blockEntityRenderState.seed + index));
                entry.renderCount = Math.min(8, stack.getCount() / 2 + 1);
                entry.slotIndex = index;
                blockEntityRenderState.itemStates.add(entry);
            }
        }
    }
    ...
    private void renderItems(BarrelBlockEntityRenderState barrel, PoseStack poseStack, @NonNull SubmitNodeCollector submitNodeCollector) {
        int globalIndex = 0;
        for (BarrelBlockEntityRenderState.ItemEntry item : barrel.itemStates) {
            if (!item.itemState.isEmpty()) {
                for (int i = 0; i < item.renderCount; i++) {
                    poseStack.pushPose();

                    float x = stableRandom(barrel.seed, globalIndex, item.slotIndex + 1) * 0.4f;
                    float z = stableRandom(barrel.seed, globalIndex, item.slotIndex + 2) * 0.4f;
                    float y = (float) (globalIndex / 4) * 0.025f + stableRandom(barrel.seed, globalIndex, item.slotIndex + 3) * 0.05f;
                    y += (float) (Math.sin(barrel.time / 10f + globalIndex) * 0.02f);

                    float yRot = stableRandom(barrel.seed, globalIndex, item.slotIndex + 4) * 5f;
                    float zRot = stableRandom(barrel.seed, globalIndex, item.slotIndex + 5) * 360f;
